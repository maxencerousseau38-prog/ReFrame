...
| **Component Library** 🌱 | `src/components/design-system/` fondée (barrel + `sections/` + README = catalogue/gouvernance/pipeline d'intake). Intake #001 (Ethereal Beams Hero, three.js) : décomposé, rejets documentés (beams/glow/gradient/lucide), extraits **`GlassPillNav`**+**`StatGroup`** (universels), section **`HeroReframed`** monochrome. Galerie `/design-system` | hue-scan 0/280, overflowX=0, 0 erreur (1440&390), 497 tests |
| V3-2..7 | CARTES chrome en verre 24px (`rounded-3xl`, result/dashboard/wizard) · Button `light` glow cleanup · motion/micro-interactions (fade/translateY/scale 0.98→1) · chat AI premium · publish flow · assets pro (zéro placeholder) · responsive parfait (ex-UX4) · unifier CTA bespoke landing (au GO) · intakes suivants (Inputs/Cards/Dialogs/Command palette/Pricing/…) | à faire, 1 lot cohérent/fois (preuve avant/après) |
